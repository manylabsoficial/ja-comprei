...
export default function BottomNav() {
    ...
    return (
        <nav
            className="fixed inset-x-0 bottom-0 z-50 border-t border-border-subtle bg-surface-overlay/95 px-2 pt-1.5 shadow-[0_-12px_40px_rgba(0,0,0,0.22)] backdrop-blur-xl"
            style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 8px)' }}
            aria-label="Navegação principal mobile"
        >
            <div className="mx-auto flex h-[62px] max-w-md items-end justify-between">
                {navItems.map((item) => {
                    ...
                    if (item.isMain) {
                        return (
                            <button
                                ...
                                className="flex min-w-0 flex-1 -translate-y-2 flex-col items-center gap-1 text-gold-500 transition-transform active:scale-95"
                    ...
                    return (
                        <button
                            key={item.id}
                            onClick={() => navigate(item.path)}
                            className={`relative flex min-w-0 flex-1 flex-col items-center justify-center gap-1 pb-1 text-[10px] font-bold transition-colors ${active ? 'text-gold-500' : 'text-text-tertiary'}`}
                            aria-label={item.label}
                            aria-current={active ? 'page' : undefined}
                        >
