-- ========================================================
-- JÁ COMPREI - MASTER SETUP COMPLETO (SCHEMA + MANYLABS SETUP)
-- ========================================================
...
DECLARE
  v_status text;
BEGIN
  -- A. Garante o Perfil Local do Já Comprei com créditos iniciais (10)
  INSERT INTO jacomprei.profiles (id, credits_balance, subscription_tier)
  VALUES (p_user_id, 10, 'user')
  ON CONFLICT (id) DO NOTHING;

  -- B. Checa se já possui acesso cadastrado no Manylabs
  ...
  -- C. Se não existir, autoativa na base unificada
...
-- Wrapper: Has App Access (Service Role apenas)
...
-- Wrapper: Current User Has App Access (Seguro, público para RLS)
...
DROP POLICY IF EXISTS users_can_view_own_profile ON jacomprei.profiles;
CREATE POLICY users_can_view_own_profile ON jacomprei.profiles
    FOR SELECT TO authenticated USING (auth.uid() = id AND public.current_user_has_manylabs_app_access());

DROP POLICY IF EXISTS users_can_update_own_profile ON jacomprei.profiles;
CREATE POLICY users_can_update_own_profile ON jacomprei.profiles
    FOR UPDATE TO authenticated USING (auth.uid() = id AND public.current_user_has_manylabs_app_access());

-- RLS para jacomprei.recipes
DROP POLICY IF EXISTS users_can_view_own_recipes ON jacomprei.recipes;
CREATE POLICY users_can_view_own_recipes ON jacomprei.recipes
    FOR SELECT TO authenticated USING (auth.uid() = user_id AND public.current_user_has_manylabs_app_access());

DROP POLICY IF EXISTS users_can_insert_own_recipes ON jacomprei.recipes;
CREATE POLICY users_can_insert_own_recipes ON jacomprei.recipes
    FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id AND public.current_user_has_manylabs_app_access());

DROP POLICY IF EXISTS users_can_update_own_recipes ON jacomprei.recipes;
CREATE POLICY users_can_update_own_recipes ON jacomprei.recipes
    FOR UPDATE TO authenticated USING (auth.uid() = user_id AND public.current_user_has_manylabs_app_access());

DROP POLICY IF EXISTS users_can_delete_own_recipes ON jacomprei.recipes;
CREATE POLICY users_can_delete_own_recipes ON jacomprei.recipes
...
DROP POLICY IF EXISTS users_can_view_own_metadata ON jacomprei.user_recipe_metadata;
CREATE POLICY users_can_view_own_metadata ON jacomprei.user_recipe_metadata
    FOR SELECT TO authenticated USING (auth.uid() = user_id AND public.current_user_has_manylabs_app_access());

DROP POLICY IF EXISTS users_can_insert_own_metadata ON jacomprei.user_recipe_metadata;
CREATE POLICY users_can_insert_own_metadata ON jacomprei.user_recipe_metadata
    FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id AND public.current_user_has_manylabs_app_access());

DROP POLICY IF EXISTS users_can_update_own_metadata ON jacomprei.user_recipe_metadata;
CREATE POLICY users_can_update_own_metadata ON jacomprei.user_recipe_metadata
    FOR UPDATE TO authenticated USING (auth.uid() = user_id AND public.current_user_has_manylabs_app_access());

-- RLS para jacomprei.shopping_lists
DROP POLICY IF EXISTS users_can_view_own_lists ON jacomprei.shopping_lists;
CREATE POLICY users_can_view_own_lists ON jacomprei.shopping_lists
    FOR SELECT TO authenticated USING (auth.uid() = user_id AND public.current_user_has_manylabs_app_access());

DROP POLICY IF EXISTS users_can_insert_own_lists ON jacomprei.shopping_lists;
CREATE POLICY users_can_insert_own_lists ON jacomprei.shopping_lists
    FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id AND public.current_user_has_manylabs_app_access());

DROP POLICY IF EXISTS users_can_update_own_lists ON jacomprei.shopping_lists;
CREATE POLICY users_can_update_own_lists ON jacomprei.shopping_lists
    FOR UPDATE TO authenticated USING (auth.uid() = user_id AND public.current_user_has_manylabs_app_access());

DROP POLICY IF EXISTS users_can_delete_own_lists ON jacomprei.shopping_lists;
CREATE POLICY users_can_delete_own_lists ON jacomprei.shopping_lists
    FOR DELETE TO authenticated USING (auth.uid() = user_id AND public.current_user_has_manylabs_app_access());

-- RLS para jacomprei.credit_transactions
DROP POLICY IF EXISTS users_can_view_own_transactions ON jacomprei.credit_transactions;
CREATE POLICY users_can_view_own_transactions ON jacomprei.credit_transactions
    FOR SELECT TO authenticated USING (auth.uid() = user_id AND public.current_user_has_manylabs_app_access());

DROP POLICY IF EXISTS users_can_insert_own_transactions ON jacomprei.credit_transactions;
CREATE POLICY users_can_insert_own_transactions ON jacomprei.credit_transactions
    FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id AND public.current_user_has_manylabs_app_access());
